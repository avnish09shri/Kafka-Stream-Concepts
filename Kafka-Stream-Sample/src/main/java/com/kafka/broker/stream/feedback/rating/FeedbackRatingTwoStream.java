...
import com.kafka.broker.message.FeedbackRatingTwoMessage;
...
public class FeedbackRatingTwoStream {
    ...
    public KStream<String, FeedbackMessage> kStreamFeedBackRating(StreamsBuilder builder){
        ...
        var feedbackStream = builder.stream("t-commodity-feedback",
                Consumed.with(stringSerde, feedbackSerde));

        var feedbackRatingStateStoreName = "feedbackRatingTwoStateStore";

        var storeSupplier = Stores.inMemoryKeyValueStore(
                feedbackRatingStateStoreName);

        var storeBuilder = Stores.keyValueStoreBuilder(
                storeSupplier,
                        stringSerde, feedbackRatingTwoStoreValueSerde);

        builder.addStateStore(storeBuilder);

        feedbackStream.transformValues(() -> new FeedbackRatingTwoValueTransformer(feedbackRatingStateStoreName),
                        feedbackRatingStateStoreName)
