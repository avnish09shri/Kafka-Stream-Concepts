...
public class FeedbackSixStream {
    ...
    public KStream<String, FeedbackMessage> kStreamFeedback(StreamsBuilder builder){
        ...
        sourceStream.flatMap(splitWords()).split()
                .branch(isGoodWord(),
                        Branched.withConsumer(ks -> { ks.repartition(Repartitioned.as("t-commodity-feedback-six-good"))
                ...
                .branch(isBadWord(),
                        Branched.withConsumer(ks -> { ks.repartition(Repartitioned.as("t-commodity-feedback-six-bad"))
